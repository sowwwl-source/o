...
function sowwwl_first_forwarded_value(string $headerValue): string
{
    $parts = preg_split('/\s*,\s*/', trim($headerValue)) ?: [];
    $candidate = trim((string) ($parts[0] ?? ''));
    return sowwwl_is_valid_ip($candidate) ? $candidate : '';
}

function sowwwl_effective_client_ip(?array $server = null): string
{
    $server = is_array($server) ? $server : $_SERVER;
    ...
    if (!sowwwl_is_trusted_proxy($remoteAddr, $server)) {
        return $remoteAddr;
    }

    foreach (['HTTP_CF_CONNECTING_IP', 'HTTP_X_FORWARDED_FOR'] as $headerName) {
        $candidate = sowwwl_first_forwarded_value((string) ($server[$headerName] ?? ''));
        if ($candidate !== '') {
            return $candidate;
        }
    }
