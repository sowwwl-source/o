...
usage() {
	cat <<'EOF'
Usage:
  sudo bash scripts/install_pi_vision_service.sh [options]
...
Examples:
  sudo bash scripts/install_pi_vision_service.sh --user pi
  sudo bash scripts/install_pi_vision_service.sh --user pablo --no-start
  sudo bash scripts/install_pi_vision_service.sh --user pi --env-example systemd/sowwwl-pi-vision.pi2.env.example
EOF
}
